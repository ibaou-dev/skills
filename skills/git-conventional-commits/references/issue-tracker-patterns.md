...
When CONTRIBUTING.md is found, scan it for these signals to identify the tracker type and extract format rules.
...
- GitHub recognizes: `close`, `closes`, `closed`, `fix`, `fixes`, `fixed`, `resolve`, `resolves`, `resolved` (case-insensitive)
...
**CRITICAL:** Never use `#N` for Jira. The key is the full `PROJECT-NUMBER` string.
...
- If ambiguous, use whatever key format appears in `git log` examples in CONTRIBUTING.md
...
- Treat scope as **mandatory** if CONTRIBUTING.md says "required" or "must include"
- If the change doesn't fit any listed scope, pick the nearest or omit and note why
...
- Custom footer tokens (some projects use `BREAKING:` instead of `BREAKING CHANGE:`)
...
Use the **exact keyword casing** found in CONTRIBUTING.md. If CONTRIBUTING.md says `Fixes`, use `Fixes` not `fixes`.
...
2. Infer tracker from remote URL:
   ...
   - `atlassian.net` → Jira (but you can't know the project key — omit issue reference or ask user)
3. If no remote URL or ambiguous → use `Closes #N` as generic default
4. **Always state explicitly:** "No CONTRIBUTING.md found — using standard CC spec with [GitHub/GitLab/inferred] issue format"
